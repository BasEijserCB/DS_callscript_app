javascript:(function(){
  ...
  fetch(SRC+'?t='+Date.now(),{cache:'no-store'})
    ...
    .then(function(code){
      ...
      t.innerHTML='↻ <b>Extra rijtijd '+getVersion(code)+' gedownload</b><br><span style="font-weight:400;">Nu actief: '+getVersion(cached)+' — druk opnieuw om de nieuwe versie te laden</span><button onclick="this.parentNode.remove()" style="position:absolute;top:8px;right:10px;background:none;border:none;color:#fff;font-size:18px;cursor:pointer;line-height:1;opacity:0.8;">×</button>';
      t.style.cssText='position:fixed;bottom:20px;right:20px;z-index:1000001;background:#1a7f37;color:#fff;padding:12px 36px 12px 14px;border-radius:8px;font:13px "Segoe UI",sans-serif;box-shadow:0 4px 16px rgba(0,0,0,.35);max-width:300px;line-height:1.5;';
