...
- **De optielijst komt uit de pagina, niet uit een tabel hier.** De dxTagBox `Depots` in het filterpaneel van de Ritmonitor heeft alle 116 depots met hun id al in zijn `dataSource`; `depotOpties()` leest die uit. Let op: **de ids zijn strings** (`"13"`), anders dan de TagBoxen in het Import-formulier, waar altijd `[parseInt(id)]` moet.
- **Van 116 naar 20: alleen stamdepots.** `GEEN_STAMDEPOT` gooit weg wat geen bezorgdepot is — netwerkvarianten (`BuiltIn-…`, `1M…`, `1M Installation …`, `Tilburg 2M (BE)`), fietsdepots (`Fietshub …`, `Bikedepot …`), warehouses en interne leveringen. Het is een regel en geen namenlijst, zodat een nieuw stamdepot vanzelf meekomt. Daarnaast `UITGESLOTEN_DEPOTS`: bakken die er als stamdepot uitzien maar het niet zijn, en dat aan de naam niet laten zien. `Amsterdam` en `Venlo (DE)` blijven leeg; de drie `WAD…`-depots (Waddeneilanden) zijn puur administratief.
- **`DEPOTS`: land én coördinaten**, 20 regels. Het land houdt de keuze binnen de grens (nazorg gaat niet over de grens); de coördinaten voeren de straal. Het zijn **stadscoördinaten, niet de depotadressen** — reken op een afwijking van een kilometer of tien, wat bij een straal van 75 km ruim binnen de marge valt. Een naam die niet in `DEPOTS` staat blijft handmatig aanvinkbaar maar doet niet mee in de automatische keuze: we weten niet waar hij ligt.
...
- **Vangnet.** Wordt de TagBox niet gevonden (`depotBox()` zoekt op het label `Depots` — nadrukkelijk niet `Begin depot` / `Eind depot` — met als terugval de box waarvan de waarde gelijk is aan het huidige filter), dan verdwijnt het blok uit het paneel en wordt er geen depotfilter meegestuurd.
...
De 10 stamdepots die in de keuze verschijnen: **NL** Almere · Deventer · Groningen · Rotterdam · Tilburg · Utrecht · Venlo (NL) — **BE** Antwerpen · Gent · Nivelles. De tien Duitse (Dusseldorf, Hamburg, Hamm, Kelsterbach, Langenhagen, Leipzig, Nurnberg, Schonefeld, Tamm, Troisdorf) staan wel in `DEPOTS` maar doen niet mee.
