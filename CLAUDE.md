...
5. **Bereken.** In deze volgorde: adres geocoderen (PDOK, anders Nominatim) → land bepalen → depots kiezen binnen `DEPOT_STRAAL_KM` → `GetTours` met die depots → eigen rit eruit → netwerkfilter op de ritnaam → stops per overgebleven rit → de `MAX_ROUTE_RITTEN` dichtstbijzijnde door de router → ranglijst. Het depotblok toont intussen *"Automatisch gekozen: …"*, en het invulblok klapt aan het eind dicht tot de samenvattingsbalk.
6. **Uitslag lezen**: de ranglijst in het paneel, de kolom `+ rijtijd` in de stoplijst van de geselecteerde rit, en het pilletje als het paneel klein staat.
...
- **Het filterpaneel wordt niet bediend.** De tool zet de TagBox niet en drukt niet op Filteren; hij overschrijft alleen het veld `depots` in de kopie van `staticFilter` die `haalTours()` toch al voor zijn eigen request bouwt. Het scherm van de gebruiker verandert dus niet. Dat het óók via de UI kan is met `probe-depotfilter.js` vastgesteld (`inst.option('value', […])` + `#filter-submit` laat de hele keten `onFilter → loadTours → GetTours` lopen), maar die weg heeft zichtbare bijwerkingen en is niet gekozen.
