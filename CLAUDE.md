...
- **De keuze valt in `scan()`, ná het geocoderen.** Daar zijn de coördinaten van de nazorg bekend, dus `autoDepots(punt, land, eigenRit)` kan rekenen. Drie regels, in deze volgorde:
  ...
  3. **Ondergrens `MIN_DEPOTS`** (3): ook als er niets binnen de straal ligt. In Zeeland en Zuid-Limburg is er geen enkel depot binnen 75 km; zonder die ondergrens zocht de tool daar in één depot. Die ondergrens heeft wel een plafond, `DEPOT_MAX_KM` (100 km): anders sleepte de derde plek in Vlissingen en Maastricht Utrecht mee op 128 respectievelijk 144 km, waar nooit een rit vandaan komt. Gevolg van juist 100 en niet 110: in Zeeland, Friesland en Twente blijft alléén het eigen depot over, want de eerstvolgende buur ligt daar op 100 tot 108 km (Vlissingen → Tilburg 106, Leeuwarden → Almere 100,2, Enschede → Venlo 107). Dat is bewust: die regio's worden in de praktijk door één depot bediend. Het plafond geldt alleen voor de ondergrens, niet voor de straal zelf. Ligt zelfs het dichtstbijzijnde depot erbuiten, dan doet dat ene toch mee — leeg zou 'alle depots' betekenen.
...
- **Wat het oplevert** (adres + eigen rit → gekozen depots): Landsmeer `NLAL` → Almere, Utrecht, Rotterdam · Maastricht `NLTI` → Tilburg, Venlo · Terneuzen → Rotterdam, Tilburg · Emmen `NLGR` → Groningen, Deventer · Den Helder `NLAL` → Almere, Utrecht · Vlissingen `NLRO` → alleen Rotterdam · Leeuwarden `NLGR` → alleen Groningen · Enschede `NLDE` → alleen Deventer · Antwerpen → alle drie de BE-depots.
