...
| `tourtool/probe-depotfilter.js` | Recon van het filterpaneel: inventariseert de tagboxes en knoppen, leest optielijsten uit (`__opties`), en kan een filter zetten en op Filteren drukken. Waarmee vastgesteld is dat het depotfilter in de `filter=`-parameter van `GetTours` landt. Naslag; niet nodig in productie. |
...
| Versie | Wijziging |
|---|---|
| v1.42.3 | Fix: het ordernummer werd op het achtste cijfer afgekapt. Coolblue is in sept. 2026 voorbij 99.999.999 gegaan, dus het middendeel van `A-100152904-511379217` telt negen cijfers, en `/\d{8}/` pakte daar `10015290` uit — een bestaande maar verkeerde order in kolom H, in `orderNr` (`-DS`) op het Import-formulier en in de parkeersleutel. Nu `/\d{8,}/` op alle drie de plekken: de consumer portal (`OrderNumberTransport`), Basic (`Pakbonnummer` / `Order nr. verlader` / `Afnemer nummer`) en de DOM-poll die op het ordernummer wacht. Het achterste deel van de referentie komt niet mee, want de eerste cijferreeks wint. |
...
**Alleen NL en BE (v1.15.0).** `ONDERSTEUNDE_LANDEN`. Duitsland doet nadrukkelijk niet mee: de dekking daar is zo anders opgezet dat de aannames van deze tool — stamdepot per stad, vaste tourduur, de netwerken 1M/1X/2M/BI — er niet opgaan. Dat is op drie plekken afgedwongen: de Duitse depots verschijnen niet in de depotkeuze, een adres in DE wordt door `scan()` geweigerd vóór er iets opgehaald wordt, en mocht het depotfilter ooit falen dan vallen Duitse ritten alsnog uit de kandidatenlijst. De DE-depots blijven wél in `DEPOTS` staan — die tabel beschrijft wat er bestáát, en zonder die regels zouden ze als "land onbekend" juist altijd zichtbaar zijn.
...
**Router: OpenRouteService, met OSRM als vangnet — nu ook als ORS uitvalt (v1.17.0).** `matrix()` kiest op `ORS_KEY` (in `localStorage`, niet in het bestand). Is die gevuld, dan gaat het naar OpenRouteService (`POST /v2/matrix/driving-car`, sleutel in de `Authorization`-header); is hij leeg, dan naar `router.project-osrm.org`. Allebei leveren `durations[i][j]` in seconden, dus de rest van de tool merkt het verschil niet.

Dat onderscheid bestaat omdat de OSRM-demoserver door het OSRM-project uitdrukkelijk bestemd is voor ontwikkelen en testen, niet voor dagelijks operationeel gebruik. Zolang `ORS_KEY` leeg is, meldt de tool dat zelf onder de uitslag — die afwijking hoort niet stil te zijn. De sleutel staat in `localStorage` en wordt via een veld in het paneel gezet; bij een gratis sleutel is dat te overzien en hij is opnieuw te genereren.

**Het vangnet werkt maar één kant op, en dat is geen omissie.** Faalt ORS, dan neemt OSRM het over. Andersom kan niet: is er geen sleutel, dan *is* OSRM al de keuze en bestaat er geen tweede engine. Een mislukking is daar het eindstation.

De keten zit in `probeerRouter()`, met drie regels die de moeite waard zijn:

- **Welke engine leverde, staat onder de uitslag** (`rijtijden via ORS`), en een uitwijk krijgt een eigen `⚑`-melding. ORS en OSRM rekenen niet gelijk, dus stil wisselen zou een onverklaarde sprong in de rijtijden geven.
- **`engineWeg()` scheidt "de engine is wég" van "déze aanvraag ging mis".** Een netwerkfout, 5xx, 429, 403 of 401 zet de engine voor de rest van de scan op dood, zodat de vijf volgende kandidaten niet elk opnieuw dezelfde mislukking afwachten. Een 400 op één set punten doet dat niet — die zegt niets over de volgende rit.
- **Vallen beide engines weg, dan noemt de melding ze allebei.** Alleen de laatste fout tonen stuurt je naar de verkeerde dienst.

**Bewust géén schattingsmodus.** Bij het vallen van beide engines zou de tool de ranglijst kunnen vullen met `afstand × KM_NAAR_MIN` — die schatting stuurt de voorselectie toch al. Dat is op 11-09-2026 afgewezen: zo'n lijst ziet er precies zo uit als een echte uitslag, en er ligt buiten de tool al een werkwijze voor als hij het niet doet. Liever een tool die eerlijk stilstaat.

**Aanleiding (11-09-2026).** `api.openrouteservice.org` (129.206.5.53) weigerde verbindingen — `ERR_CONNECTION_REFUSED` binnen 29 ms, óók vanaf de commandline, terwijl `openrouteservice.org` zelf in hetzelfde subnet gewoon 200 gaf en PDOK, Nominatim en OSRM allemaal werkten. Een storing bij HeiGIT, niet bij Coolblue. Omdat `matrix()` een harde `ORS_KEY ? ors : osrm` was, viel de hele tool stil terwijl OSRM antwoordde.

**De foutmelding wees daarbij naar de verkeerde oorzaak.** Elke `Failed to fetch` werd gemeld als *"Netwerkverzoek geblokkeerd (waarschijnlijk CSP)"*, terwijl de Ritmonitor helemaal geen CSP-header stuurt — er was niets te blokkeren. De melding noemt nu de onderliggende fout en zet de oorzaken op volgorde van waarschijnlijkheid: dienst plat, netwerk, en pas daarna CSP. **Les:** een browser meldt alleen *dát* een fetch faalde; wie daar één oorzaak van maakt in de UI, stuurt de volgende lezer een dag het bos in. Wil je het echt weten, dan is `document.addEventListener('securitypolicyviolation', …)` het enige sluitende antwoord — geen melding bij een mislukte call betekent geen CSP.

**Een GAS-proxy is overwogen en niet gekozen (11-09-2026).** `UrlFetchApp` zou CORS, CSP en preflights volledig wegnemen en de ORS-sleutel serverkant houden. Maar hij lost het probleem hierboven níét op: ORS weigerde iedereen, ook Google. Vier kosten wogen zwaarder: `fetch` kan de output van een GAS web-app cross-origin niet lezen (zie v1.35.0), dus het zou JSONP worden — GET-only, geen `Authorization`-header; GAS voegt een cold start plus een hop toe, maal zes matrixcalls, in een tool die antwoord moet geven terwijl er iemand aan de lijn hangt; het koppelt de rijtijd-tool aan hetzelfde GAS-project dat het onverklaarde gat van 21 uur opleverde (incident 10/11-08-2026); en één gedeelde sleutel betekent één gedeeld dagquotum, terwijl nu iedereen zijn eigen limiet meebrengt. De optie blijft geldig zodra blijkt dat Coolblue's netwerk een routerhost blokkeert — dán is het precies het juiste gereedschap.

Voor later: OSRM zelf draaien (container met een NL+BE-extract) lost naleving, privacy én beschikbaarheid in één keer op, en is sneller dan een gedeelde server. Dat blijft de echte oplossing; het ontbreekt aan een host.
...
**Rekenregels.** Per gat: `t(A→X) + t(X→B) − t(A→B)`, alle drie via dezelfde engine (publieke OSRM-demoserver). Alleen gaten vanaf de huidige positie van de bezorger tellen mee: de laatste stop met een echte `RealArrivalDatestamp` (de ↑/↓-pijltjes in de lijst) is waar de rit nu is. Ritten met **voorsprong** (gepland min werkelijk) komen boven: `netto = max(0, benodigde tijd − voorsprong)`. Voorafgaand aan het routeren wordt geschift, want elke kandidaat kost één `matrix()`-aanroep: `MAX_ROUTE_RITTEN` (6) gaan er echt de router in. Zie **Voorselectie**.
...
**Welke ritten meedoen.** Het depotfilter bepaalt welke ritten überhaupt opgehaald worden; daarna draaien nog vier schiftingen vóór het ophalen van de stops, zodat er geen tientallen onnodige requests uitgaan:

0. **Depots** (v1.5.0). De tool stuurt zijn eigen `depots`-lijst mee in de `filter=`-parameter van `GetTours` en is daarmee niet meer afhankelijk van wat de gebruiker in de Ritmonitor heeft aangevinkt — precies het punt: een naburig depot kan ook dichtbij zijn en veel voorsprong hebben. Zie **Depotfilter** hieronder.
...
2. **Ritten zonder gat, uit de rittenlijst zelf (v1.10.0).** `GetTours` geeft per rit `NumberOfVisits` en `NumberOfVisitsCompleted`; blijven er minder dan twee over, dan kan er geen gat meer zijn en gaat de `GetVisits` niet uit. Die tellingen bevatten ook activiteiten (pauzes, depothandelingen), dus ze zijn niet gelijk aan wat `verwerkStops()` overhoudt — maar de fout valt de veilige kant op: minder dan twee rijen betekent zeker minder dan twee stops.
3. **Al verlengde ritten (v1.14.0).** `teLang()`: duurt een rit langer dan de maximale tourduur van zijn land (`TOURDUUR`, plus `TE_LANG_MARGE` van 5 min tegen afrondingsruis), dan is daar eerder op de dag al iets aan toegevoegd — vrijwel altijd door DS zelf. Zo'n rit valt af als kandidaat, **hoe groot de voorsprong ook is**: die voorsprong is dan geen ruimte maar het gevolg van dat eerdere ingrijpen. De norm werkt hier als plafond voor élke configuratie, dus het startvenster speelt geen rol; Duitse ritten hebben geen norm en vallen daarom nooit af.

   Zo'n rit wordt ook uit `maakCohort()` gehouden. Anders werd hij de langste van zijn cohort en leken al zijn collega's ineens te kort — één rit van 535 minuten zou elke normale rit van 488 een "ruimte" van 47 minuten geven.
4. **Ritten buiten NL/BE** vallen af (v1.15.0). Vangnet voor het geval het depotfilter faalt; normaal komen ze er niet eens in.
5. **Noodrem `MAX_VISIT_RITTEN`** (60, v1.10.0). Normaal blijven er tientallen ritten over en gaan ze er alle in, maar met een handvol depots aangevinkt kan dat ver boven de honderd lopen — evenzoveel `GetVisits`-calls op DireXtion. Boven de grens winnen de ritten met de meeste voorsprong, want dat is ook de eerste sleutel van de ranglijst. Die voorsprong komt uit `voorsprongUitLijst()`: `TimelinessMinutes` uit de rittenlijst, waarvan het teken **niet uit het getal maar uit het label `Timeliness`** wordt afgeleid — bij een rit met label `TooEarly` zagen we `-43`, en of `TooLate` dan positief is, is nooit geverifieerd. Hoeveel ritten er afvielen staat onder de uitslag.
6. **Netwerkvinkjes.** Vier vinkjes, één per netwerk — `1M` (één man, begane grond), `1X` (één man installateur, inbouw maar begane grond), `2M` (twee man, tilt naar boven), `BI` (twee man installatie, kan alles). Aangevinkt = die ritten mogen de aftercare doen. De keuze wordt direct in `localStorage` (`rijtijd_netwerken`) bewaard. Bewust handmatig: het bronnetwerk van de klant zegt wat die ploeg kón, niet wat de aftercare nódig heeft, en die vertaling zit nog in niemands hoofd op papier.

**Gebruikersflow (v1.5.0).** Wat de medewerker doet, en wat de tool daarop doet:

1. **In het DS Logboek**, bij de uitkomstvraag: knop *"Adres klaarzetten voor reistijd-check"*. Publiceert adres, taak, `dienstType`, `formaatTV`, `route` en `orderBron` naar `localStorage` én het klembord.
2. **Op de Ritmonitor**: rijtijd-bookmarklet klikken. Het paneel opent met het invulblok open.
3. **Knop *"↓ Adres uit DS Logboek"***. Vult adres, eigen rit, servicetijd (uit `TAKEN`) en de netwerkvinkjes (uit `TAKEN[taak].netwerken`). Het depotblok toont dan nog niets aangevinkt, met de melding dat de depots bij Bereken vanzelf gekozen worden.
...
- **De keuze valt in `scan()`, ná het geocoderen.** Daar zijn de coördinaten van de nazorg bekend, dus `autoDepots(punt, land, eigenRit)` kan rekenen. Drie regels, in deze volgorde:
  ...
  3. **Ondergrens `MIN_DEPOTS`** (3): ook als er niets binnen de straal ligt. In Zeeland en Zuid-Limburg is er geen enkel depot binnen 75 km; zonder die ondergrens zocht de tool daar in één depot. Die ondergrens heeft wel een plafond, `DEPOT_MAX_KM` (110 km): anders sleepte de derde plek in Vlissingen en Maastricht Utrecht mee op 128 respectievelijk 144 km, waar nooit een rit vandaan komt. **110 en niet 100**, hoewel dat laatste even ingesteld heeft gestaan (v1.10.1): de eerstvolgende buur ligt in de dunne regio's net rond die grens — Vlissingen → Tilburg 106, Leeuwarden → Almere 100,2, Enschede → Venlo 107 — en op 100 hielden die drie alleen hun eigen depot over. Dat zou vaak kloppen (Tilburg rijdt Zeeland zelf), maar een buurdepot te veel kost een handvol `GetVisits` en er een missen kost de oplossing. Het plafond geldt alleen voor de ondergrens, niet voor de straal zelf. Ligt zelfs het dichtstbijzijnde depot erbuiten, dan doet dat ene toch mee — leeg zou 'alle depots' betekenen.
...
- **Wat het oplevert** (adres + eigen rit → gekozen depots): Landsmeer `NLAL` → Almere, Utrecht, Rotterdam · Maastricht `NLTI` → Tilburg, Venlo · Vlissingen `NLRO` → Rotterdam, Tilburg · Leeuwarden `NLGR` → Groningen, Almere, Deventer · Enschede `NLDE` → Deventer, Venlo · Terneuzen → Rotterdam, Tilburg · Den Helder `NLAL` → Almere, Utrecht · Antwerpen → alle drie de BE-depots.

**Het lijstje in het paneel is een overrule, geen invulveld.** Het toont wat de tool koos; vink je zelf iets aan, dan blijft die keuze staan tot je een ander adres invult of op "automatisch" klikt. Er wordt niets bewaard in `localStorage` — een depotkeuze van gisteren zegt niets over de nazorg van vandaag. Zelfde regel als bij de uitslagen, zie **Elke sessie begint leeg**. Handmatig met niets aangevinkt betekent: geen depotfilter, alle ritten.

- **`DEPOTS` is de bron: id, land en coördinaten**, 20 regels waarvan 10 in de keuze verschijnen. Het land houdt de keuze binnen de grens (nazorg gaat niet over de grens); de coördinaten voeren de straal; het id gaat mee in de `filter=`-parameter. Het zijn **stadscoördinaten, niet de depotadressen** — reken op een afwijking van een kilometer of tien, wat bij een straal van 75 km ruim binnen de marge valt. De ids zijn **strings** (`"13"`), anders dan de TagBoxen in het Import-formulier, waar altijd `[parseInt(id)]` moet.
- **Waarom een tabel en niet de pagina (v1.16.0).** Tot v1.16.0 las `depotOpties()` naam en id uit de dxTagBox `Depots` van de Ritmonitor, om geen lijst te hoeven onderhouden — met `GEEN_STAMDEPOT` en `UITGESLOTEN_DEPOTS` om er de 116 ingangen mee terug te brengen tot 20. Die TagBox bleek in de praktijk niet betrouwbaar te vinden: drie verschillende DOM-aannames faalden achter elkaar (label als vorige sibling, label in het bovenliggende element, de inhoud van de `dataSource`), en telkens viel de hele depotfiltering stil weg. Tien ids in een tabel die toch al land, coördinaten en routecode bijhoudt is die kwetsbaarheid niet waard. `depotOpties()` leest nu uit `DEPOTS` — geen DOM, geen dataSource, geen timing.

  **Let op: v1.16.0 was stuk.** De zoek-en-vervang die deze wijziging aanbracht gebruikte een eindmarkering die niet meer bestond en viel terug op een markering veel verderop, waardoor ruim duizend regels — 51 functies — uit het bestand verdwenen. `node --check` en `build.py` merkten dat niet: het bleef syntactisch geldig. v1.16.1 is de goede build. Les: bij een grote scriptmatige wijziging de functie-inventaris vóór en ná vergelijken, en een diff van 1055 verwijderde tegen 55 toegevoegde regels nooit blind pushen.

  Komt er een depot bij, dan hoort het in `DEPOTS`. Het id is op te halen met het recept in `probe-depotfilter.js` (`__opties(<i>)` op het depotveld). Dat is een handmatige stap die niemand eraan herinnert — de prijs voor een depotkeuze die altijd werkt.
- **De TagBox is nu bijzaak.** `depotBox()` bestaat nog, maar alleen om het filter in de Ritmonitor mee te zetten (`zetRitmonitorFilter()`). Lukt dat niet, dan klopt de uitslag nog steeds en verschijnt de bestaande `⚑`-melding.
- **Land van de nazorg** komt uit `landVanNazorg()`: de ritcode (`2M-NLTI-07` → NL, exact) en anders de postcode in het adresveld, met dezelfde regel als het logboek gebruikt (vijf cijfers = DE, vier = BE, anders NL).
- **Twee sporen, en dat is met opzet (v1.6.0).** `haalTours()` doet zijn eigen `GetTours` met de gekozen depots — dat is waar de uitslag op draait. Daarnaast zet `zetRitmonitorFilter()` diezelfde keuze in de Ritmonitor zelf en drukt op `#filter-submit`, zodat tool en gebruiker naar hetzelfde kijken. Dat tweede is nodig omdat klikken op een regel in de uitslag `selectTourId()` aanroept, en een rit die niet in de rittenlijst staat niet te selecteren is: zoeken in Rotterdam terwijl het scherm op Tilburg filtert geeft anders een uitslag waar je niet doorheen kunt klikken. Het zetten gaat via de **TagBox eerst** (v1.11.0), met de knockout-observables in `tourFilter` als terugval. Die volgorde is niet willekeurig: de TagBox-route is met `probe-depotfilter.js` bewezen — waarde zetten plus `#filter-submit` laat de hele keten lopen mét de depots in de URL — terwijl van `staticFilter.depots` nooit is vastgesteld dát het een observable is. Daarna leest de functie de waarde terug; klopt die niet, dan wordt er niet geklikt, want dan zou de Ritmonitor op zijn oude keuze filteren en verandert er alleen iets op het scherm. Na de klik wacht `scan()` 900 ms op de eigen `loadTours` van DireXtion. Het blijft **best effort**: mislukt het, dan is er niets stuk want de zoektocht draait op het eigen request — maar er verschijnt nu wel een `⚑`-melding onder de uitslag, want stil falen is precies hoe dit lang onopgemerkt bleef.

**Bug in v1.6.0–v1.10.2 (opgelost in v1.11.0):** één gedeelde vlag `gezet` besloeg alle negen velden. Was `staticFilter.depots` géén observable maar bijvoorbeeld `stateFilter.finishedTours` wél, dan stond die vlag op true, werd het TagBox-vangnet overgeslagen, en werd er op Filteren gedrukt met de **oude** depots er nog in. Elk veld wordt nu apart beoordeeld.
- **Geen enkel ander filter mag versmallen.** Beide sporen zetten `shippers`, `tags`, `searchTags` en `depotBeginsWith` leeg en `finishedTours`/`inactiveTours` op `show`, `tourProblems`/`timeliness` op `allTours`. Een verlader- of uitvoerderfilter dat nog van een eerdere zoektocht in de Ritmonitor stond, mag de beste rit niet stil buiten beeld houden. Alleen `date` blijft staan — dat is geen versmalling maar de dag zelf.
- **`depotBox()` zoekt in vier stappen**, want op één signaal leunen ging herhaaldelijk mis: label precies `Depots` · label bevat `depot` · de inhoud verraadt het (de box die meerdere namen uit `DEPOTS` kent, in DOM-volgorde) · de box waarvan de waarde het huidige filter is. `Begin depot` en `Eind depot` vallen bij elke stap af. Wordt hij niet gevonden, dan blijft alleen het zetten van het schermfilter achterwege.

**Het depotfilter is een stadskeuze.** Alle netwerken van een depot vallen tegenwoordig onder de naam van de stad: alle Tilburgse ritten, van 1M tot BI, zitten in `Tilburg` (id 13). De ingangen `BuiltIn-Tilburg`, `1MTilburg` en `1M Installation Tilburg` zijn resten van een oudere inrichting en blijven leeg. Het netwerkonderscheid leeft daarmee alleen nog in de **ritnaam** (`2M-NLTI-07`), waar `netwerkVan()` het uit haalt.

De 10 stamdepots die in de keuze verschijnen, met hun id: **NL** Almere `2021191` · Deventer `696250` · Groningen `4885` · Rotterdam `1721` · Tilburg `13` · Utrecht `14` · Venlo (NL) `421593` — **BE** Antwerpen `18808` · Gent `35210` · Nivelles `696230`. De tien Duitse staan wel in `DEPOTS` maar doen niet mee.
...
Tot v1.8.0 werd puur op hemelsbrede afstand geschift. Dat botste met de ranglijst erna, die "past het binnen de voorsprong" als eerste sleutel heeft: een rit met veertig minuten voorsprong op negen kilometer viel af voor zes ritten om de hoek zonder speling, terwijl hij de lijst zou hebben aangevoerd. Twee verschillende vragen, en de beste rit viel ertussenuit.

De shortlist bestaat nu uit twee bakken:
...
**Wat er per rit staat, en wat bewust niet (v1.7.0).** Een regel toont: ritnaam · uitloop · het gekozen gat (`6 → 7` met de twee straten) · de opbouw · de voorsprong. Tot v1.7.0 stond daaronder nog een technische regel (`31 → 35 min rijden · 1.4 km · vanaf stop 2 (2 gehad)`). Die is weg: het waren drie ongelijksoortige getallen zonder label op één regel, en vooral misleidend, want die `1.4 km` was de afstand tot de díchtstbijzijnde toekomstige stop — het getal waarop de ritten voorgesorteerd worden — en niet de afstand bij het getoonde gat. De velden `afstand`, `vanafSeq` en `overgeslagen` zijn daarmee ook uit het resultaatobject verdwenen.
...
**Twee maatstaven, in deze volgorde (v1.13.0).**

**1. De standaard tourduur, voor ritten die tussen 07:45 en 09:00 beginnen.** `TOURDUUR` is 490 minuten in NL en 475 in BE; `ruimte = norm − geplande duur`. Alleen in dat startvenster: later op de dag bestaan er praktisch geen standaardritten, en de aangepaste routes daar hebben elk hun eigen maximum omdat de latere start van de tourduur af gaat. Het land komt uit `MobileGroupCode` in de rittenlijst, met de landletters van de routecode (`2M-NLTI-07` → NL) als terugval.

**Maar niet elke rit in dat venster is een standaardrit (v1.13.1).** De losploeg lost eerst trailers en gaat daarna pas bezorgen; die routes beginnen óók in de ochtend en zijn met reden korter. Zonder correctie zou de tool ze elke dag als "korte rit" flaggen. Ze zijn te herkennen zonder tabel, via het axioma dat een andere starttijd altijd een andere configuratie is: `normDuur()` kijkt naar de **langste rit met dezelfde starttijd binnen hetzelfde depot** (alle netwerken samen, de index `cohort.start`). Ligt die meer dan `NORM_MARGE` (60 min) onder de norm, dan is dat de configuratie en geen tekort — de norm geldt dan niet en maatstaf 2 neemt het over. De losploegtijden verschillen per depot, dus dit moest uit de data komen; in Tilburg op 09-09-2026 was 07:50 de standaard (17 ritten, langste 488) en 08:05 de losploeg (2 ritten, langste 312).

**Prijs daarvan:** een rit met een unieke starttijd in het venster staat in een groep van één, en dan is zijn eigen duur de langste — kort betekent dan geen norm en dus geen melding. Bewust die kant op: liever een korte rit gemist dan de losploeg elke dag ten onrechte geflagd.

De norm bevestigt zichzelf in de data: de langste 07:50-ritten in Tilburg duren 486–488 minuten. En hij ziet wat het cohort niet kon zien — `1X-NLTI-01` (07:50–14:21) is de enige 1X-rit van die starttijd, dus er was niets om mee te vergelijken, terwijl hij 99 minuten onder de norm zit. Dat werkt omdat de startgroep waar hij in valt (NLTI @ 07:50, alle netwerken) wél vol is.

**Openstaand: avondritten.** Die beginnen in NL overal om 13:20 en bestaan niet in BE, maar hun standaardduur is niet bekend. Ze vallen daarom onder maatstaf 2. In de Tilburgse steekproef waren het er twee (7,5 en 7,8 u), elk de enige van hun netwerk met die starttijd — dus geen signaal.

**2. Het cohort, voor al het andere.** Binnen één netwerk hoort een starttijd bij precies één configuratie; wil de planning twee configuraties met dezelfde starttijd, dan wordt er intern een minuut verschil gezet. Ritten met hetzelfde netwerk én dezelfde starttijd horen dus even lang te duren. `ruimteVan()` neemt de langste rit van dat cohort als maat: `ruimte = langste − eigen duur`, en pas vanaf `RUIMTE_MIN` (10 min) heet dat iets. Alles komt uit `PlanStartDatestamp` en `PlanEndDatestamp` in de rittenlijst — geen extra request.

**Waarom niet de mediaan per netwerk**, wat de eerste opzet van maatstaf 2 was: die beweegt mee met de groep. In Tilburg zijn vier van de zeven 1M-ritten kort, dus de mediaan zakt naar 5,5 u — waarna een rit van 4,2 u nog maar 78 min "ruimte" krijgt en een volle rit van 7,9 u als *"144 min langer"* wordt gemeld, alsof die overladen is.
...
**Het blijft een aanwijzing, geen zekerheid, en dat is in de UI te zien.** Past de klus binnen de voorsprong, dan staat er een groene pil `past in de voorsprong` — dat is zeker en hoeft niemand na te lopen. Past hij alleen dankzij de korte rit, dan staat er een amberen pil `⚠ korte rit` plus een blok eronder: *"Korte rit — controleer dit. Deze rit staat 31 min korter gepland dan de standaard tourduur van 490 min."* — of, bij maatstaf 2, *"…dan de andere 2M-ritten die om 07:50 beginnen."* `ruimteBron` bepaalt welke van de twee. Die melding verschijnt alléén als er op de ruimte geleund wordt; past het al op de voorsprong, dan zou hij ruis zijn.
...
**Elke sessie begint leeg (v1.18.0).** De tool bewaart geen uitslagen meer. Het paneel opent zonder ranglijst, zonder kolom in de stoplijst, zonder pil en met een leeg adresveld; een mislukte berekening laat óók niets achter.

**De scheidslijn: instellingen blijven, bevindingen niet.** Wat een keuze van de gebruiker is overleeft de sessie — de netwerkvinkjes (`rijtijd_netwerken`) en de ORS-sleutel (`rijtijd_ors_key`). Wat een uitkomst voorstelt niet. De depotkeuze viel altijd al aan die goede kant, om dezelfde reden.

**Waarom.** Tot v1.17.0 werden `rijtijd_resultaten` en `rijtijd_adres` bij het openen van het paneel teruggehaald, en bleven ze staan als een scan faalde. Bij de ORS-storing van 11-09-2026 zag je dus een rode foutbalk met daaronder een complete ranglijst — ritnamen, gaten, minuten — van een vórige nazorg. Zo'n lijst leest als een antwoord, ongeacht wat erboven staat, en dat is precies hoe iemand een toezegging doet op een berekening die nooit voor dit adres is gemaakt. Een niet-werkende tool is veiliger dan een tool die oud werk als nieuw presenteert. De prijs is een keer extra op Bereken, en dat is bewust geaccepteerd.

`wisUitslag()` doet de opruiming en wordt op drie plekken aangeroepen: bovenaan `scan()` (zodat élke uitweg daaronder — geweigerd land, mislukte geocode, router eruit — een leeg paneel achterlaat), door de Wissen-knop, en impliciet bij het opstarten doordat er niets meer te laden valt. Het is bewust één routine: een hálve opruiming is het gevaar, want een achtergebleven kolom of pil hoort dan bij een zoektocht die niet meer bestaat. Vouwen doet hij niet — `scan()` wil het formulier laten staan, de Wissen-knop wil het openklappen, dus dat bepaalt de aanroeper.

De twee oude sleutels worden bij het opstarten actief verwijderd (`vergeet(KEY_RES); vergeet(KEY_ADRES);`), zodat er geen dode data achterblijft in de browser van iedereen die met een oudere versie gewerkt heeft — de fout die `ds_log_buffer` nog steeds in het veld houdt.

**Bij het opstarten staat het invulblok altijd open (v1.4.1).** Tot v1.4.0 begon het paneel compact als er nog bewaarde resultaten in `localStorage` stonden — en dan zat de knop "Adres uit DS Logboek" verstopt in het dichtgeklapte blok, terwijl dat juist het eerste is wat je nodig hebt als je de tool opent. Dichtklappen is nu uitsluitend het gevolg van een berekening die je zojuist deed, niet een toestand waarin je belandt. `vouwForm(false)` staat daarom nog op precies één plek: aan het eind van een geslaagde `scan()`. Sinds v1.18.0 bestaan die bewaarde resultaten sowieso niet meer, dus de situatie uit v1.4.0 kan niet meer ontstaan.
